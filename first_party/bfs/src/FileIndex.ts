...
export class FileIndex<T> {
      ...
      for (const node in tree) {
        if (Object.prototype.hasOwnProperty.call(tree, node)) {
          ...
          if (children) {
            ...
            inode = new IndexFileInode<Stats>(
              new Stats(FileType.FILE, -1, 0x16d),
            );
          }
          if (parent) {
  ...
  public addPathFast(path: string, inode: IndexInode): boolean {
    ...
    const parentPath =
      itemNameMark === 0 ? '/' : path.substring(0, itemNameMark);
 ...
 * @hidden
 */
export function isIndexFileInode<T>(
  inode: IndexInode | null,
): inode is IndexFileInode<T> {
 ...
 * @hidden
 */
export function isIndexDirInode<T>(
  inode: IndexInode | null,
): inode is IndexDirInode<T> {

...
export interface MountMapping {
  [point: string]: InstanceType<ProviderConstructor>;
}
 ...
 */
export function resolveFS(path: string): {
  fs: FileSystem;
  path: string;
  mountPoint: string;
} {
  ...
  ); // decending order of the string length
  ...
  throw new ApiError(
    ErrorCode.EIO,
    'BrowserFS not initialized with a file system',
  );
}
...
export function fixError<E extends Error>(
  e: E,
  paths: { [from: string]: string },
): E {
  console.log(e);
...
export function initialize(mountMapping: MountMapping): void {
  ...
  for (const [point, fs] of Object.entries(mountMapping)) {
    const FS = fs.constructor as unknown as ProviderConstructor;
    if (!FS.isAvailable()) {
      throw new ApiError(
        ErrorCode.EINVAL,
        `Can not mount "${point}" since the filesystem is unavailable.`,
      );
    }

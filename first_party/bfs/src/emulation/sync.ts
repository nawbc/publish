import type { ReadSyncOptions, symlink } from 'fs';
...
function doOp<M extends FileSystemMethod, RT extends ReturnType<M>>(
  ...[name, resolveSymlinks, path, ...args]: Parameters<M>
): RT {
  path = normalizePath(path!);
  const { fs, path: resolvedPath } = resolveFS(
    resolveSymlinks && existsSync(path) ? realpathSync(path) : path,
  );
  try {
...
export function readFileSync(
  filename: string,
  arg2: { encoding: string; flag?: string } | { flag?: string } | string = {},
): FileContents {

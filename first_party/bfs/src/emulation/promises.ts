...
type FileSystemMethod = {
  [K in keyof FileSystem]: FileSystem[K] extends (...args: any) => any
    ? (
        ...
        ...args: Parameters<FileSystem[K]>
      ) => ReturnType<FileSystem[K]>
    : never;
 ...
 * @returns
 */
async function doOp<M extends FileSystemMethod, RT extends ReturnType<M>>(
  ...[name, resolveSymlinks, path, ...args]: Parameters<M>
): Promise<RT> {
  path = normalizePath(path!);
  const { fs, path: resolvedPath } = resolveFS(
    resolveSymlinks && (await exists(path)) ? await realpath(path) : path,
  );
  try {
...
export async function open(
  path: string,
  flag: string,
  mode: number | string = 0o644,
): Promise<number> {
  const file: File = await doOp(
    'open',
    true,
    path,
    FileFlag.getFileFlag(flag),
    normalizeMode(mode, 0o644),
    cred,
  );
...
export async function writeFile(
  ...
  data: FileContents,
  options?:
    | { encoding?: string; mode?: number | string; flag?: string }
    | string,
): Promise<void>;
export async function writeFile(
  filename: string,
  data: FileContents,
  arg3?: { encoding?: string; mode?: number | string; flag?: string } | string,
): Promise<void> {
...
export async function write(
  fd: number,
  arg2: Uint8Array | string,
  arg3?: number,
  arg4?: BufferEncoding | number,
  arg5?: number,
): Promise<number> {
...
export async function realpath(
  ...
  try {
    ...
    const dst =
      mountPoint + normalizePath(await fs.readlink(resolvedPath, cred));
    return realpath(dst);
  } catch (e: any) {
    throw fixError(e, { [resolvedPath]: path });
  }

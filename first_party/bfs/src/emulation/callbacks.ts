...
export function truncate(
  path: string,
  arg2: number | BFSOneArgCallback = 0,
  cb: BFSOneArgCallback = nop,
): void {
...
export function open(
  path: string,
  flag: string,
  arg2?: number | string | BFSCallback<number>,
  cb: BFSCallback<number> = nop,
): void {
  const mode = normalizeMode(arg2, 0o644);
  cb = typeof arg2 === 'function' ? arg2 : cb;
  promises
    .open(path, flag, mode)
    .then((fd) => cb(undefined, fd))
    .catch(cb);
}
...
export function readFile(
  filename: string,
  options: { flag?: string },
  callback?: BFSCallback<Uint8Array>,
): void;
export function readFile(
  filename: string,
...
export function readFile(
  filename: string,
  arg2: any = {},
  cb: BFSCallback<string> | BFSCallback<Uint8Array> = nop,
) {
  cb = typeof arg2 === 'function' ? arg2 : cb;
...
export function writeFile(
  ...
  promises.writeFile(
    filename,
    data,
    typeof arg3 === 'function' ? undefined : arg3,
  );
}
...
export function write(
  ...
  offset: number,
  length: number,
  cb?: BFSThreeArgCallback<number, Uint8Array>,
): void;
export function write(
  fd: number,
  buffer: Uint8Array,
...
export function write(
  ...
  arg5?: any,
  cb:
    | BFSThreeArgCallback<number, Uint8Array>
    | BFSThreeArgCallback<number, string> = nop,
): void {
  ...
  if (typeof arg2 === 'string') {
    ...
    promises
      ...
      .then((bytesWritten) => _cb(undefined, bytesWritten, decode(buffer)))
      .catch(_cb);
  } else {
    ...
    const _cb = (typeof arg5 === 'function' ? arg5 : cb) as BFSThreeArgCallback<
      number,
      Uint8Array
    >;
    promises
...
export function read(
  ...
  length: number,
  position?: number,
  cb: BFSThreeArgCallback<number, Uint8Array> = nop,
): void {
  promises
...
export function symlink(
  srcpath: string,
  dstpath: string,
  arg3?: _symlink.Type | BFSOneArgCallback,
  cb: BFSOneArgCallback = nop,
): void {
  const type = typeof arg3 === 'string' ? arg3 : 'file';
  cb = typeof arg3 === 'function' ? arg3 : cb;
  promises
    .symlink(srcpath, dstpath, typeof arg3 === 'function' ? undefined : arg3)
    .then(() => cb())
    .catch(cb);
}
...
export function watch(
  filename: string,
  arg2: any,
  listener: (event: string, filename: string) => any = nop,
): FSWatcher {
  throw new ApiError(ErrorCode.ENOTSUP);
}

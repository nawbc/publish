...
export class PreloadFile<T extends FileSystem> extends BaseFile {
   ...
   */
  public override async chmod(mode: number): Promise<void> {
    this.chmodSync(mode);
  }
   ...
   * @param [Number] mode
   */
  public override chmodSync(mode: number): void {
   ...
   * @param [Number] gid
   */
  public override async chown(uid: number, gid: number): Promise<void> {
   ...
   * @param [Number] gid
   */
  public override chownSync(uid: number, gid: number): void {
    if (!this._fs.metadata.supportsProperties) {
      throw new ApiError(ErrorCode.ENOTSUP);
    }
    this._dirty = true;

...
import { encode, randomUUID, ROOT_NODE_ID } from '../utils';
...
export class AsyncKeyValueFile
...
export class AsyncKeyValueFileSystem extends BaseFileSystem {
  ...
  public override async rename(
    ...
    try {
      ...
      if (newDirList[newName]) {
        ...
        const newNameNode = await this.getINode(
          tx,
          newPath,
          newDirList[newName],
        );
  ...
  public override async createFile(
    ...
    const tx = this.store.beginTransaction('readwrite'),
      ...
      newFile = await this.commitNewFile(
        tx,
        p,
        FileType.FILE,
        mode,

...
export class SyncKeyValueFileSystem extends SynchronousFileSystem {
  ...
  public override accessSync(p: string, mode: number, cred: Cred): void {
  ...
  public override createFileSync(
    p: string,
    flag: FileFlag,
    mode: number,
    cred: Cred,
    ...
    if (parent === '/') {
      ...
      return readDirectory(
        this.getINode(
          tx,
          parent + sep + filename,
          this._findINode(tx, dirname(parent), basename(parent), visited),
        ),
      );
    }
  }
